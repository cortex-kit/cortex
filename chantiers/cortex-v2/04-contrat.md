...
- §9, forme des hooks : `scaffold.py` livre `.claude/hooks/session_start.py` et `.claude/hooks/stop.py` (stdlib, auto-testés) et `settings.json` les appelle par `python3 .claude/hooks/<nom>.py`. `session_start.py` lance `lint_sante.py --bref` et le rappel de clôture ; `stop.py` rend un `systemMessage` de clôture si `git status --porcelain` n'est pas vide. La recette vérifie ces scripts, pas une commande inline.
- §5, compteur : en `mode: solo`, l'étape 8 vaut `arbitre` ; le compteur d'un atelier solo complet dit 8 faites et 1 arbitrée, jamais 9/9. La page vierge et `LISEZ-MOI.html` comptent 8 pastilles « À faire », l'étape courante étant en tête hors tableau.
- §4, chemins : `scan.py` écrit la forme `~` pour tout chemin sous le dossier personnel ; un dépôt cloné hors de `~` (recette lancée depuis un dossier temporaire) produit des chemins absolus, et la recette ne l'exige alors pas.
- §6, §9, interface publique de `lint_sante.py` : `parse_frontmatter(texte)`, `en_liste(valeur)`, `liens_sortants(texte)`, `lint(vault, ...)` et la constante `DURS` sont des entrées publiques, consommées par `federe.py`. Toute modification de leur signature se fait avec la fédération. Le critère « stdlib seule » des audits s'entend hors imports internes au dépôt (`cortex_config`, `lint_sante`, `etat`).
- §6, `config.yaml` du commun : `federe.py` écrit un fichier qui satisfait `valider_installable` de la lane C : `profil: societe`, `mode: federe`, `commun.racine: ""` (le commun n'a pas de commun), un bloc `donnees` avec `regime: pointeur` (le commun ne copie rien, il agrège). Le contrôle `commun_edite_main` ne se déclenche que sur un `commun.racine` non vide, ce qui reste vrai.
- §2, §3, slug : le slug est `organisation.code` ; le maillon 0 le demande (« quel nom court pour votre second cerveau ? ») et l'écrit dans `organisation.code` du `config.yaml` qu'il crée. `~/Cortex/<slug>/`, `_export/<slug>/`, `cortex-<slug>` et `federation.yaml` utilisent cette valeur, et aucune autre.
- I10, prose : aucun SKILL.md ne porte plus « Sur accord, lancer `cortex-N+1` » ; seule la section Notice propose la suite. Les hooks lisent le vault dans `CLAUDE_PROJECT_DIR` et retombent sur le dossier courant.
- §1, possession : `skills/stop-slop/` appartient à la lane B ; `chantiers/cortex-v2/consignes-<lane>.md`, `rapport-<lane>.md` et `audit-<lane>.md` sont des livraisons du chef d'orchestre, même quand le commit part d'une branche de lane. `rejeu_profil.py` vit dans `skills/cortex-4-installation/recette/` (outil de recette, hors des maillons livrés), possédé par G et alimenté par C.
- §2, `collecte` : deux clés nouvelles pour `scan.py` : `max_extractions: 40` (plafond de conversions par `markitdown`, distinct de `sante.max_structurants` qui reste le plafond de copie du maillon 5) et `budget_secondes: 120` (durée totale du scan au-delà de laquelle il s'arrête proprement et déclare `bornes.depassement`). `.lnk` n'est pas un signal de base déportée. `graphify_propose` ne vaut `true` que pour un dépôt dont `langages` n'est pas vide, une seule fois par sous-arbre.
- §3, `poste.py` : le bloc `poste.outils` liste tout le kit avec `present` vrai ou faux, comme l'exemple du §3 ; `organisation.code` (le slug) est écrit dans `config.yaml` et `poste.json` par le maillon 0. `notice.py` pose `notice_ouverte_le` à chaque régénération. Le zip de repli n'embarque pas `cortex-4-installation/recette/`. Tout script du dépôt doit passer `ast.parse` sous Python 3.9 (aucun antislash dans une expression f-string).
- §4, rejeu du scan : si `--out` existe, `scan.py` relit le JSON et ne remplace que `disque`, `depots`, `bornes`, `racines`, `extraction`, `genere_le` ; `mail`, `bases`, `agenda`, les `resume`, `preuve_de` et les écarts dérivés par l'agent sont conservés. `source_id` est unique entre racines (préfixé du dernier segment du parent quand deux racines portent le même nom).
- Rejeu de cadrage (recette) : `rejeu_profil.py` estampille `00-cadrage.md` en `statut: brouillon` avec les contrôles humains (`racines_confirmees`, `plafonds_acceptes`, `mail_optin_trace`) à `arbitre`, motif « rejeu sans personne », comme il le fait déjà pour `02-ontologie.md` (invariant I11).
- §4, rejeu du scan : `format` et `version` sont réécrits par `scan.py` avec les six clés du scripteur ; les écarts candidats sont conservés par type (`base_deportee_non_declaree` et `depot_non_declare` viennent du disque et sont recalculés, les autres viennent de l'agent et sont gardés).
- §9, `allow` : dix-huitième entrée `Bash(python3 .claude/skills/ingest/copie_structurant.py:*)`, pour que le rafraîchissement d'un structurant périmé ne demande aucune permission. `scaffold.py --outillage-seul` ignore `__pycache__` et tout fichier non texte du gabarit.
- §6, sceau du commun : le README généré porte la date deux fois (clé `genere_le` en en-tête, phrase du contrat dans le corps) ; le sceau écarte ces deux lignes reconnues à leur début. Toute note d'un export absente de son `index.json` est signalée, quelle que soit sa visibilité.
- §9, racines symboliques : pour chaque racine de `collecte.racines`, `scaffold.py` écrit la forme donnée et, si elle diffère, la forme réelle (`realpath`) dans `deny` et `additionalDirectories` (constat M4 : `/var/folders` → `/private/var`). Le gabarit du vault porte un `.gitignore` (`__pycache__/`, `*.pyc`, `.DS_Store`, `:memory:.ses`) pour que le hook Stop ne compte pas les caches Python.
- I10, tranché après l'audit final (2026-09-19) : la Notice propose, le maillon n'enchaîne jamais, en solo comme en consultant. Aucun maillon ne pose « On enchaîne ? » ; il termine par la régénération de la notice et une phrase du type « quand vous voulez continuer, dites « <phrase canonique> » ». `doctrine.md` et `cortex-1-cadrage/SKILL.md` reprennent cette lecture ; la recette vérifie l'absence de « On enchaîne » et d'un appel de skill dans les fins de maillon.
- §3, retour de la Phase H (2026-09-20) : `graphify` entre dans le kit du maillon 0 (`uv tool install "graphifyy[pdf,office]"`, optionnel, jamais sur le vault) ; `options_proposees` ne garde que `wispr-flow`, `superwhisper`, `noota`. Le maillon 0 présente chaque outil manquant avant la question, et le choix vaut accord : ce qui est coché s'installe aussitôt, sans seconde confirmation.
