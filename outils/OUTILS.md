...
| markitdown | convertit Word, PDF, tableurs, présentations en texte pour l'inventaire ; appel `uvx --from "markitdown[all]" markitdown`, le paquet nu ne lit ni pdf ni docx | `uv tool install "markitdown[all]"` | idem | idem | MIT | github.com/microsoft/markitdown | installé, sondé par `uvx --from "markitdown[all]" markitdown --version` |
